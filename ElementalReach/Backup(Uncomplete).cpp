...
const int intensityOfDeviationOfFireBall = 12; //How far off target it can be.
...
const float killParticlesCollisionLeverage = 35;
...
const int fireElementalStartingPositionX = 700; //This is the starting position relative to the SCREEN.
const int fireElementalStartingPositionY = 700; //As above.
...
const float rateOfCameraMove = 3;

//POWER ELEMENTAL INFO:
const int maxPowerParts = 100;
const int powerElemSize = 40;
const int maxKilledParts = 100;
const float powerElementalStartPositionX = 1200;
const float powerElementalStartPositionY = 1200;
...
public:
    ...
    void startClock() { if(referenceTime == 0) referenceTime = SDL_GetTicks(); } //An inherit check.
...
public:
    ...
    void moveCameraWhere(float posX, float posY);
    //Could've been one function. \/
    void getCurrentScreenXval(float GCSXV) { currentScreenXVal = GCSXV; }
    void getCurrentScreenYval(float GCSYV) { currentScreenYVal = GCSYV; }
...
protected:
    ...
    float currentScreenXVal;
    float currentScreenYVal;
...
void cinematicView::moveCameraWhere(float posX, float posY)
{
    designatedX = posX;
    designatedY = posY;
    ...
    float adjustedScreenXVal = currentScreenXVal+SW/2;
    float adjustedScreenYVal = currentScreenYVal+SH/2;

    obliqueDistance = getRadius((designatedX-adjustedScreenXVal), (designatedY-adjustedScreenYVal));

    calculateDirectionX = designatedX - adjustedScreenXVal;
    calculateDirectionY = designatedY - adjustedScreenYVal;
...
float cinematicView::moveCameraX(float screenXVal)
{
   float adjustedScreenXVal = screenXVal+SW/2;
    ...
    if(!(adjustedScreenXVal > designatedX-10 && adjustedScreenXVal < designatedX+10) && fullyInCinematicView && !designationReached)
    {
        screenXVal += calculateDirectionX*rateOfCameraMove;
    }

    else if((adjustedScreenXVal > designatedX-10 && adjustedScreenXVal < designatedX+10))
    {
        designationReached = true;

        if(deactOnDesignationReach && startedMovementYet)
            active = false;
...
float cinematicView::moveCameraY(float screenYVal)
{
    ...
    if(!(adjustedScreenYVal > designatedY-10 && adjustedScreenYVal < designatedY+10) && fullyInCinematicView && !designationReached)
    {
        screenYVal += calculateDirectionY*rateOfCameraMove;
    }

    else if((adjustedScreenYVal > designatedY-10 && adjustedScreenYVal < designatedY+10))
    {
        designationReached = true;
...
cinematicView::cinematicView()
{
    ...
    currentScreenXVal = SW/2;
    currentScreenYVal = SH/2;
...
*/
    ...
    scrollControl.CV.moveCameraWhere(FEx->getPositionX(),FEx->getPositionY());
